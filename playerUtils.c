...
void userMove(Move **board, Move player) {
  ...
  while(input == 0) {
    ...
    if(input < 1 || input > 9) {
      ...
      board[(input-1)/3][(input-1) % 3] = player; //Fix row to include mod 3 so that it fits the right row as without mod 3 it would fail and go to not access the correct box
      return;
    }
    input = 0;
  }
